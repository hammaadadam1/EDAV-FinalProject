...
lea_class_data$`Artist Popularity` <- ifelse(lea_class_data$artist_name %in% top_ten_artists$Var1, "DSI Top 10", "DSI Not Top 10")
lea_class_data$`Song Popularity` <- ifelse(lea_class_data$track_name %in% top_fifty_songs$Var1, "DSI Top 50", "DSI Not Top 50")

edav_top_50_songs <- lea_class_data[which(lea_class_data$`Song Popularity` == 'DSI Top 50'), ]
edav_not_top_50_songs <- lea_class_data[which(lea_class_data$`Song Popularity` == 'DSI Not Top 50'), ]
edav_top_10_artists <- lea_class_data[which(lea_class_data$`Artist Popularity` == 'DSI Top 10'), ]
edav_not_top_10_artists <- lea_class_data[which(lea_class_data$`Artist Popularity` == 'DSI Not Top 10'), ]

edav_top_50_songs$playlist_name <- 'DSI Top 50 Songs'
edav_not_top_50_songs$playlist_name <- 'DSI Not Top 50 Songs'
edav_top_10_artists$playlist_name <- 'DSI Top 10 Artists'
edav_not_top_10_artists$playlist_name <- 'DSI Not Top 10 Artists'
...
https://github.com/LeaCollin0518/EDAV-FinalProject/blob/master/lea-DataCleaning.R
https://github.com/LeaCollin0518/EDAV-FinalProject/blob/master/Cleaning%20and%20Getting%20Album%20Data.Rmd
https://github.com/LeaCollin0518/EDAV-FinalProject/blob/master/DataImportGenre.R
...
top_genres = dsi_top_genres[1:20,] %>% left_join(top2017_genre_count, by = "Genre") %>%
              mutate(`DSI Songs`=percent.x, `Top Tracks of 2017`=percent.y) %>% ungroup() %>%
                select(Genre, `DSI Songs`, `Top Tracks of 2017`)

top_genres$`Top Tracks of 2017`[is.na(top_genres$`Top Tracks of 2017`)] = 0
...
```{r spanninggenres, include = FALSE, cache = TRUE}
...
```{r, echo = FALSE, fig.align="center"}
genre_dotplot + ggtitle("Pop Rules, but Rock Isn't Far Behind!") + 
  theme(plot.title = element_text(size=16, hjust = 0.5))
```
...
```{r, echo = FALSE, fig.align="center"}
song_heatmap + ggtitle("Who's Your Musical Soulmate?") + 
  theme(plot.title = element_text(size=16, hjust = 0.5))
```
...
```{r, echo = FALSE, fig.align="center"}
box_indie + ggtitle("Who's the Most Indie?") + 
  theme(plot.title = element_text(size=16, hjust = 0.5))
```
...
```{r, echo = FALSE, fig.align="center"}
gdiv2 + ggtitle("Tom Can't Get Enough of His Favorite Genre") + 
  ylab("Proportion of Songs in Top Genre") + 
    theme(plot.title = element_text(size=16, hjust = 0.5))
```
