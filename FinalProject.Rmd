...
```{r}
class_data <- read_csv("classMusicData.csv", col_names = TRUE)
```
...
```{r}
class_names <- read_csv("Names.csv", col_names = TRUE)
# get only the users that agreed to have their names displayed
class_names <- class_names[which(class_names$`Display?` == 'Yes'),]

class_data <- class_data %>% dplyr::inner_join(class_names, by = "user_id")
class_data$X1 <- NULL
```

```{r}
library(ggridges)
all_ridges <- ggplot(class_data, aes(x = danceability, y = Name)) + 
  geom_density_ridges2(scale = 2, fill = "#99CCFF") +
  theme_ridges() +
  scale_x_continuous(breaks = seq(min(0), max(1), by = 0.1)) +
  ylab("User")
...
```{r}
song_chart <- ggplot(top_ten_songs, aes(reorder(Var1, -Freq), Freq)) +
  geom_bar(stat = 'identity', color="#3333FF", fill="#99CCFF", width = 0.90) + 
...
```{r}
class_data$`Is Most Popular Artist?` <- ifelse(class_data$artist_name %in% top_ten_artists$Var1, "Yes", "No")
class_data$`Is Most Popular Song?` <- ifelse(class_data$track_name %in% top_ten_songs$Var1, "Yes", "No")
```

To confirm popularity and maybe learn more about how Spotify assess track popularity, let's look at the difference between track populartiy amongst the most popular songs. We'll do this by making a density curve. First, what might be interesting is to see how the features of the most popular songs in EDAV compare to the features of the most popular songs on Spotify that are in the 'United States Top 50' or 'Global Top 50' for example. 
...
```{r}
us_top_50$`Is Most Popular Song?` <- 'US Top 50'
us_top_50$`Is Most Popular Artist?` <- 'US Top 50'
us_top_50$Name <- 'USTop50'

global_top_50$`Is Most Popular Song?` <- 'Global Top 50'
global_top_50$`Is Most Popular Artist?` <- 'Global Top 50'
global_top_50$Name <- 'GlobalTop50'

top_2017$`Is Most Popular Song?` <- 'Top Tracks 2017'
top_2017$`Is Most Popular Artist?` <- 'Top Tracks 2017'
top_2017$Name <- 'TopTracks2017'
...
We now need to combine all the data together. I'm just going to put it all in a new data frame since the top 50 charts won't have all the same columns and I want to keep the class data all together and consistent. 

```{r}
excluded_vars <- c('user_id', 'location2017', 'nationality', 'undergrad', 'age', 'useFrequency', 'Display?')
sub_class <- select(class_data, -excluded_vars)
all_data <- rbind(sub_class, us_top_50)
all_data <- rbind(all_data, global_top_50)
all_data <- rbind(all_data, top_2017)
```

```{r}
diff_pop_songs <- ggplot(all_data, aes(x = track_popularity, fill = `Is Most Popular Song?`)) + 
  geom_density(alpha = 0.4) + xlab("Track Popularity")
diff_pop_songs
```

```{r}
diff_pop_artists <- ggplot(all_data, aes(x = track_popularity, fill = `Is Most Popular Artist?`)) + 
  geom_density(alpha = 0.4) + xlab("Track Popularity")
diff_pop_artists
```
...
```{r}
diff_energy_songs <- ggplot(all_data, aes(x = energy, fill = `Is Most Popular Song?`)) + 
  geom_density(alpha = 0.4) + xlab("Energy")
diff_energy_songs
```

```{r}
diff_dance_songs <- ggplot(all_data, aes(x = danceability, fill = `Is Most Popular Song?`)) + 
  geom_density(alpha = 0.4) + xlab("Danceability")
diff_dance_songs
```

```{r}
diff_loud_songs <- ggplot(all_data, aes(x = normalized_loudness, fill = `Is Most Popular Song?`)) +
  geom_density(alpha = 0.4) + xlab("Loudness")
diff_loud_songs
```

```{r}
diff_speech_songs <- ggplot(all_data, aes(x = speechiness, fill = `Is Most Popular Song?`)) +
  geom_density(alpha = 0.4) + xlab("Speechiness")
diff_speech_songs
```

```{r}
diff_valence_songs <- ggplot(all_data, aes(x = valence, fill = `Is Most Popular Song?`)) +
  geom_density(alpha = 0.4) + xlab("Valence")
diff_valence_songs
```

```{r}
diff_acoustic_songs <- ggplot(all_data, aes(x = acousticness, fill = `Is Most Popular Song?`)) +
  geom_density(alpha = 0.4) + xlab("Acousticness")
diff_acoustic_songs
```

```{r}
diff_tempo_songs <- ggplot(all_data, aes(x = normalized_tempo, fill = `Is Most Popular Song?`)) +
  geom_density(alpha = 0.4) + xlab("Tempo")
diff_tempo_songs
```

```{r}
pop_ridges <- ggplot(all_data, aes(x = danceability, y = `Is Most Popular Song?`)) + 
  geom_density_ridges2(scale = 2, fill = "#99CCFF") +
  theme_ridges() +
  scale_x_continuous(breaks = seq(min(0), max(1), by = 0.1)) 
pop_ridges
