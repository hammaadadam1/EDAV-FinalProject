...
subset(class_playlists, grepl("_", artist_name)==TRUE)$artist

song_data <- class_playlists %>% 
...
song_freq <- class_playlists %>% group_by(track_name, artist_name)  %>%
              summarise(Freq = n()) %>%
                arrange(desc(Freq))

artist_freq <- class_playlists %>% count(artist_name, sort = TRUE)
View(artist_freq)

artist_per_playlist <- class_playlists %>% distinct(Name, artist_name) %>% 
...
artist_by_listener <- class_playlists %>% distinct(Name, artist_name, track_name) %>% 
                        group_by(artist_name, Name) %>% 
                          summarise(n = n())

song_user <- class_playlists %>% select(Name, track_name, artist_name) %>% cbind(1) %>% 
              rename(binaryid="1") %>% 
